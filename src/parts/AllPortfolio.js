...
export default function AllPortfolio({ data }) {
  const mobile = data.filter((item) => item.type === 'Mobile Apps' || item.type === 'Website / Mobile Apps');

  const website = data.filter((item) => item.type === 'Website' || item.type === 'Website / Mobile Apps');
